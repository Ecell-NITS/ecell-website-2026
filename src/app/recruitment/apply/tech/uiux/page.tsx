...
export default function TechUIUXFormPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", scholarId: "", domainOfPriorProject: "", resumeLink: "", projectFigmaLink: "", prototypeFigmaLink: "", designInspirationLink: "" });
  ...
  const validateStep1 = () => {
    ...
    else if (!isValidUrl(form.projectFigmaLink)) e.projectFigmaLink = "Invalid URL";
    
    if (!form.prototypeFigmaLink.trim()) e.prototypeFigmaLink = "Required";
    else if (!isValidUrl(form.prototypeFigmaLink)) e.prototypeFigmaLink = "Invalid URL";
    
    if (!form.designInspirationLink.trim()) e.designInspirationLink = "Required";
    else if (!isValidUrl(form.designInspirationLink)) e.designInspirationLink = "Invalid URL";
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = { 
        ...
        githubProfileLink: form.projectFigmaLink || "https://github.com/uiux-dummy"
      };
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 sm:px-6 pt-8 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
        <h1 className="text-3xl font-light tracking-tight text-white sm:text-4xl">UI/UX Design</h1>
        <p className="mt-3 text-sm font-light text-gray-500">Provide your information to apply for the UI/UX team.</p>
      </motion.div>
      <div className="relative mt-12">
        <motion.div key="step1" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.5, ease: "easeOut" }} className="space-y-6">
          <FormField label="Name" required error={errors.name}><input className={inputClassName} value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Jane Doe" /></FormField>
          ...
          <FormField label="Domain of any project" hint="Optional"><input className={inputClassName} value={form.domainOfPriorProject} onChange={(e) => update("domainOfPriorProject", e.target.value)} placeholder="e.g. Graphic Design, Motion Graphics" /></FormField>
          ...
          <FormField label="Project Developer Figma / Framer Link" required error={errors.projectFigmaLink}><input className={inputClassName} value={form.projectFigmaLink} onChange={(e) => update("projectFigmaLink", e.target.value)} placeholder="https://figma.com/..." /></FormField>
          
          <FormField label="Prototype Figma / Framer Link" required error={errors.prototypeFigmaLink}><input className={inputClassName} value={form.prototypeFigmaLink} onChange={(e) => update("prototypeFigmaLink", e.target.value)} placeholder="https://figma.com/..." /></FormField>
          
          <FormField label="Design Inspiration Document Link" required hint="Add the video walkthrough link in this document itself" error={errors.designInspirationLink}><input className={inputClassName} value={form.designInspirationLink} onChange={(e) => update("designInspirationLink", e.target.value)} placeholder="https://docs.google.com/..." /></FormField>
