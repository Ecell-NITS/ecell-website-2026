...
export default function TechWebFormPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", scholarId: "", whatsappNumber: "", domainOfPriorProject: "", resumeLink: "", githubProfileLink: "", taskPreviewLink: "", taskGithubRepoLink: "", taskSelection: "" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = { ...form, techDomain: "WEB" };
      if (!payload.taskPreviewLink) delete (payload as any).taskPreviewLink;
      if (!payload.taskGithubRepoLink) delete (payload as any).taskGithubRepoLink;
      if (!payload.taskSelection) delete (payload as any).taskSelection;

      await api.post("/api/recruitment/apply/tech/submit", payload);
      localStorage.removeItem("ecell_tech_draft");
      toast.success("Application submitted!"); 
      router.push("/recruitment/success?type=TECH");
    } catch (err: unknown) { 
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 sm:px-6 pt-8 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
        <h1 className="text-3xl font-light tracking-tight text-white sm:text-4xl">Web Development</h1>
        <p className="mt-3 text-sm font-light text-gray-500">Provide your information to apply for the tech team.</p>
      </motion.div>
      <div className="relative mt-12">
        <motion.div key="step1" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.5, ease: "easeOut" }} className="space-y-6">
          ...
          <FormField label="Domain of Prior Project" hint="Optional"><input className={inputClassName} value={form.domainOfPriorProject} onChange={(e) => update("domainOfPriorProject", e.target.value)} placeholder="e.g. Web Dev, App Dev, UI/UX" /></FormField>
          <FormField label="Resume Link" required hint="Viewable URL" error={errors.resumeLink}><input className={inputClassName} value={form.resumeLink} onChange={(e) => update("resumeLink", e.target.value)} placeholder="https://..." /></FormField>
          <FormField label="GitHub Profile Link" required error={errors.githubProfileLink}><input className={inputClassName} value={form.githubProfileLink} onChange={(e) => update("githubProfileLink", e.target.value)} placeholder="https://github.com/..." /></FormField>
